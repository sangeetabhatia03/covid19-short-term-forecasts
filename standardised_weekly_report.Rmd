...
at least five deaths were observed in the country in the last week and 10 in the 
preceding week.

* We forecast deaths as it is likely that reporting of deaths in more
...
quantifiable with delay of up to a three weeks.


A **secondary** objective of this report is to analyse case
ascertainment per country. As well as forecasting, we also use reported deaths and past reported
...
death to delayed cases would be exactly the Case Fatality Ratio (CFR).
...
* Japan with an estimated Rt of 0.6 (95% CrI 1.2 - 0.2)
* China with an estimated Rt of 0.8 (95% CrI 0.6 - 1.0)

Forecast of predicted deaths in the coming week (week starting 22nd March) is highest in:
  
* Italy with a forecasted number of weekly deaths 9030 (95% CrI 7813 - 10400)
* Spain with a forecasted number of weekly deaths 5628 (95% CrI 3950 -
  8654)

and is lowest in:
  
* Japan with a forecasted number of weekly deaths 6 (95% CrI 0 - 44)
* Iraq with a forecasted number of weekly deaths 14 (95% CrI 0 - 825).

```{r}
x <- drake::readd("weekly_predictions_qntls")
x <- na.omit(x)
propinci <- assessr:::prop_in_ci(x$observed, x$`2.5%`, x$`97.5%`)
```
Forecasts in previous weeks were good with 
`r scales::percent(propinci, accuracy = 0.1)` of observed number
...
* South Korea with an estimated ascertainment of 100% (95% CrI 89.2% - 100%)
* Germany with an estimated ascertainment of 23.1% (95% CrI 18.3% - 29.6%)

and countries with the lowest case ascertainment were:
  
* Spain with an estimated ascertainment of 1.6% (95% CrI 1.6% - 1.7%)
...
* Italy with an estimated true epidemics size in this period of
  1364565 (95% CrI 1333474 - 1395862).
* Spain with an estimated true epidemics size in this period of
1190204 (95% CrI 1149931 - 1229837)

Countries with the lowest true epidemics size in this period were:
...
We define a country to have sustained local active transmission if
at least five deaths were observed in the country in the last week and 10 in the 
preceding week. We intend to produce forecasts every week, for the
week ahead. Three different modelling teams will produce these
forecats which are then used to produce an ensemble forecasts.

Our main analysis assume a gamma distributed serial of mean 4.8 days
...
importantly, while using such longer serial interval has very little
impact on the weekly forecast produced, it results in much higher
estimated of transmissibility. The figure below shows the two serial
interval distributions that have been used in these analyses.
...
of exposure patterns during the SARS epidemic [lloyds-smith sars
nature]. 
...
The methods is very similar to that of model 1 but given uncertainty
...
estimating R, as described in [Nouvellet 2018 simple approach]. The
model has the advantage of being more robust to changes in reporting
...
As for model 1, we used the renewal equation to project the incidence
...
period. We sampled a sets of back-calculated early incidence curves
...
Projections were made on a 7-days horizon. The projections assume that
the transmissibility remains constant over this horizon. If
transmissibility were to decrease as a result of control interventions
and/or changes in behaviour over this time period, we would predict a
lower number of cases; similarly, if transmissibility were to increase
over this time period, we would predict a higher number of cases. We
...
The analysis relies on 2 key assumptions:
  
* The true underlying Case Fatality Ratio (CFR) is 1%,
* The delay from a death to this case being reported is 10 days.

First, we analyses the ratio of reported deaths to reported cases 10
days prior.Therefore, the ratio roughly accounts for the delay between death and
case being reported.  Any temporal trends suggests a change in the reporting. 
For instance, an increase in the ratio gives an indication that cases
reporting is decreasing.
...
(black). The reported deaths and cases have been rescaled to a scale
of 0 to 1 to allow for easy comparison.
...
If all cases (including asymptomatic) and deaths were reported, and
the delay from reporting to death was exactly 10 days, then the ratio
defined above would be equivalent to the Case Fatality Ratio (CFR).
