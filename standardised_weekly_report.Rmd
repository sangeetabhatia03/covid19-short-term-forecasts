...
the number of countries/regions included is 22.
...
the infection of COVID-19 fatalities. Therefore, the
...
```{r}
sicaption <- "**Figure 1**: Serial Interval distributions used in the anlyses. Here the serial interval relates to death and characterise the time between a case dying and its infector dying. Our main analysis assumes a Gamma distribution with a mean of 4.80 days and a standard deviation of 2.70 days (shown in purple). The longer serial interval, used as sentivity analysis, with a mean of 6.48 days and a standard deviation of 3.83 days is shown in green."
```
...
Ensemble models, even if built with a relatively simple apprach such
...
**Figure 2**: Reported daily death, current and past forecasts based on the ensemble model.
...
95% CrI interval), while latest forecasts are show in red (median and
...
**Figure 4**: Reported daily death, current and past forecasts based on model 1.
For each country with active transmission (see methods), we plot
the observed incidence of deaths (black dots). 
Past forecasts, where available, are shown in green (median and
95% CrI interval), while latest forecasts are show in red (median and
...
**Figure 6**: Reported daily death, current and past forecasts based on model 2.
For each country with active transmission (see methods), we plot
the observed incidence of deaths (black dots). 
Past forecasts, where available, are shown in green (median and
95% CrI interval), while latest forecasts are show in red (median and
...
**Figure 8**: Reported daily death, current and past forecasts based on the model ensemble.
For each country with active transmission (see methods), we plot
the observed incidence of deaths (black dots). 
Past forecasts, where available, are shown in green (median and
95% CrI interval), while latest forecasts are show in red (median and
...
Alos plotted are the reported deaths (red dots) and reported cases
(black dots). 
...
**Table 4**: Results summary for repoting and case ascertainment. We present 
(**i**) the estimated ratio of deaths to reported cases 10 days prior per country
  (mean and 95% CI) during the last *14 days*; (**ii**) the estimated case ascertainment per country (mean and 95% CI) during the last *14 days*; (**iii**) the estimated factor to be multiplied to observed cases number to obtain
  the true number of cases (scaling factor x observed cases). Mean and 95% CI, estimated for the last *14 days*; (**iv**) the observed and predicted number of cases in the *last week* (week starting [date]).
