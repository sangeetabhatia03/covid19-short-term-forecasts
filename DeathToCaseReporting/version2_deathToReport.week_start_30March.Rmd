...
for (i in 1:nrow(I)){
  for (j in 1:N_geo){
    ...
    summary_rho$low[i,j+1] <- quantile(rho_it[i,j,],.025,na.rm = TRUE)
    summary_rho$high[i,j+1] <- median(rho_it[i,j,],.975,na.rm=TRUE)
...
for (i in f){
  for (j in 1:N_geo){
    ...
    summary_I_true$high[i,j+1] <- median(I_true[i,j,],.975,na.rm=TRUE)
  }
}
...
summary_14days <- data.frame(
    ...
    deaths_to_reported_ratio = glue::glue(
        "{scales::percent(summary_r$median[nrow(D),-1], accuracy = 0.1)}
        ({scales::percent(summary_r$low[nrow(D),-1],accuracy = 0.1)} - {scales::percent(summary_r$high[nrow(D),-1],accuracy = 0.1)})",
    ),    
    estimated_reporting = glue::glue(
        "{scales::percent(summary_rho$median[nrow(D),-1], accuracy = 0.1)} 
        ({scales::percent(summary_rho$low[nrow(D),-1],accuracy = 0.1)} - {scales::percent(summary_rho$high[nrow(D),-1],accuracy = 0.1)})",
    ),   
    factor_to_real_size = glue::glue(
        "{round(1/summary_rho$median[nrow(D),-1], 2)} 
        ({round(1/summary_rho$low[nrow(D),-1], 2)} - {round(1/summary_rho$high[nrow(D),-1], 2)})",
    ),   
    Observed_case_last_week = colSums(tail(I[,-1],7)),
    Predicted_True_case_last_week = glue::glue(
        "{prettyNum(signif(last_week$median,digits = 2), big.mark = ",")} ",
        "({prettyNum(signif(last_week$low,digits = 2), big.mark = ",")}",
        " - {prettyNum(signif(last_week$high,digits = 2), big.mark = ",")})"
        )
  )  
...
unformatted_summary_14days <- data.frame(
    country = country,
    deaths_to_reported_ratio_50 = round(summary_r$median[nrow(D),-1],digits = 3),
    deaths_to_reported_ratio_025 = round(summary_r$low[nrow(D),-1],digits = 3),
    deaths_to_reported_ratio_975 = round(summary_r$high[nrow(D),-1],digits = 3),
    estimated_reporting_50 = round(summary_rho$median[nrow(D),-1],digits = 3),
    estimated_reporting_975 = round(summary_rho$low[nrow(D),-1],digits = 3),
    estimated_reporting_025 = round(summary_rho$high[nrow(D),-1],digits = 3),    
    factor_to_real_size_50 = round(1/summary_rho$median[nrow(D),-1],digits = 1),
    factor_to_real_size_025 = round(1/summary_rho$low[nrow(D),-1],digits = 1),
    factor_to_real_size_975 = round(1/summary_rho$high[nrow(D),-1],digits = 1),
    Observed_case_last_week = colSums(tail(I[,-1],7)),
    Predicted_True_case_last_week_50 = round(last_week$median,digits = 0),
    Predicted_True_case_last_week_025 = round(last_week$low,digits = 0),
    Predicted_True_case_last_week_975 = round(last_week$high,digits = 0)
)
readr::write_csv(
